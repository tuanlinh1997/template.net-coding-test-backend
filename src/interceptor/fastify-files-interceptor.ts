...
export function FastifyFilesInterceptor(fieldName: string, maxCount?: number, localOptions?: Options): Type<NestInterceptor> {
            ...
            await new Promise<void>((resolve, reject) =>
                this.multer.array(fieldName, maxCount)(ctx.getRequest(), ctx.getResponse(), (error: any) => {
                    if (error) {
                        ...
                        return reject(new HttpException(`${error.message}: ${fieldName}`, HttpStatus.UNPROCESSABLE_ENTITY));
                    }
                    resolve();
                }),
            );
